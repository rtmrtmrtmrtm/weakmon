...
/*
 * like fano() but no metric table, instead sym0 and sym1
 * directly provide the probabilities.
 * metrics.c suggests e.g. in0[i] is floor(0.5+4*log2(2*p0)), where p0
 * is the probability that the i'th symbol represents a zero.
 */
int
nfano(
...
int *sym0,              /* log of prob of each symbol being 0 */
int *sym1,              /* log of prob of each symbol being 1 */
...
unsigned long maxcycles)/* Decoding timeout in cycles per bit */
{
	...
	tail = &nodes[nbits-31]; // originally 33

	/* Compute all possible branch metrics for each symbol pair
	 * This is the only place we actually look at the raw input symbols
	 */
	for(np=nodes;np <= lastnode;np++){
                np->metrics[0] = sym0[0] + sym0[1];
                np->metrics[1] = sym0[0] + sym1[1];
		np->metrics[2] = sym1[0] + sym0[1];
		np->metrics[3] = sym1[0] + sym1[1];
